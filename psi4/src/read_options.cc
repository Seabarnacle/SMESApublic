    ...
    **Cfour Interface:** Keyword translates into |cfour__cfour_spherical|. -*/
    ...
    if (name == "SCF" || options.read_globals()) {
        ...
        **Cfour Interface:** Keyword translates into |cfour__cfour_scf_damping|. -*/
        ...
        /*- Do use LIST extrapolation to accelerate convergence? -*/
        options.add_str("LIST", "NONE", "NONE LISTB LISTF LISTR FDIIS LIST3");
        /*- Do use MESA to accelerate convergence? -*/
        options.add_bool("MESA", false);
        /*- MESA SCF Convergence Methods to use -*/
        options.add("MESA_SCF_METHODS", new ArrayType());
        /*- MESA error type. -*/
        options.add_str("MESA_ERROR", "INTRA_D INTER_F INTER_D D_F COMMUTATOR");
        /*- Enable open-shell specific algorithms for MESA/LIST methods. When enabled, 
        alpha and beta components are handled separately for UHF calculations. -*/
        options.add_bool("MESA_OPEN_SHELL", true);
