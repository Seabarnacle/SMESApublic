...
#endif
// orthogonalize the Fock matrix -> diagonalize it -> back-transform 
...
// NOTE here tranform
SharedMatrix HF::form_FoutmFin(SharedMatrix Fout, SharedMatrix Fin) {
    Fout->subtract(Fin);
    // Fout->transform(X_);
    return Fout;
}

SharedMatrix HF::form_MaddM(SharedMatrix M1, SharedMatrix M2) {
    M1->add(M2);
    return M1;
}

// X^T M X
SharedMatrix HF::form_XMX(SharedMatrix M) {
    M->transform(X_);
    return M;
}
