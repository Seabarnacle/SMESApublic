...
class HF : public Wavefunction {
   ...
   public:
    ...
    /** Form M1 - M2 (for LIST) **/
    virtual SharedMatrix form_FoutmFin(SharedMatrix Fout, SharedMatrix Fin);

    /** Form M1 + M2 (for LIST) **/
    virtual SharedMatrix form_MaddM(SharedMatrix M1, SharedMatrix M2);

    /** Form X'(M)X (for LIST) **/
    virtual SharedMatrix form_XMX(SharedMatrix M);
