...
class LIST:
    """
    An object to assist in the LIST interpolation procedure.
    ...
    def __init__(self, A: core.Matrix, max_vec: int = 6, list_varient: str = "LISTb",
                 removal_policy: str = "OLDEST", ):
        self.d_in = []
        self.f_in = []
        self.d_out = []
        self.f_out = []
        ...
        self.list_varient = list_varient
        self.max_vec = max_vec
        self.removal_policy = removal_policy

        if self.removal_policy not in ["LARGEST", "OLDEST"]:
            raise ValidationError("LIST: removal_policy must either be oldest or largest.")

    def add(self, d_out: core.Matrix, f_out: core.Matrix):
        """
        Adds a LIST state and error vector to the LIST object.
        ...
        f_out
            ...
        """
        self.d_out.append(d_out.clone())
        self.f_out.append(f_out.clone())

    def extrapolate(self,
                    fin: Optional[core.Matrix] = None,
                    din: Optional[core.Matrix] = None) -> core.Matrix:
        """
        Shoot the next state vector from current set of state and error vectors.
        ...
        # Limit size of LIST vector
        list_out_count = len(self.d_out)
        list_in_count = len(self.d_in)

        if list_out_count == 0:
            raise ValidationError("LIST: No previous iterations done.")
        if list_out_count == 1 and list_in_count == 0:
            f_in = self.f_out[0]
            d_in = self.d_out[0]
            self.f_in.append(f_in)
            self.d_in.append(d_in)
            del self.f_out[0]
            del self.d_out[0]
            return f_in, d_in
        if list_out_count > self.max_vec:
            pos = 0
            del self.d_in[pos]
            del self.d_out[pos]
            del self.f_in[pos]
        ...
        B = np.empty((list_out_count + 1, list_out_count + 1))
        B[0, :] = -1
        B[:, 0] = -1
        B[0, 0] = 0

        front_element = self.d_out
        if self.list_varient == "LISTb":
            front_element = self.d_out
        elif self.list_varient == "LISTf":
            front_element = self.f_in

        for i, e in enumerate(front_element):
            for k in range(len(self.f_in)):
                f_diff = self.f_out[i].clone()
                f_diff.subtract(self.f_in[i])
                B[i + 1, k + 1] = (core.triplet(self.A, e, self.A, False, False, False).vector_dot(
                    core.triplet(self.A, f_diff, self.A, False, False, False)))

        # build O vector
        resid = np.zeros(list_out_count + 1)
        resid[0] = -1
        ...
        if fin is None:
            fin = core.Matrix("LIST F result", self.f_out[0].rowdim(), self.f_out[0].coldim())
            din = core.Matrix("LIST D result", self.d_out[0].rowdim(), self.d_out[0].coldim())
        else:
            fin.zero()
            din.zero()

        for num, c in enumerate(ci[1:]):
            fin.axpy(c, self.f_out[num])
            din.axpy(c, self.d_out[num])

        self.f_in.append(fin.clone())
        self.d_in.append(din.clone())
