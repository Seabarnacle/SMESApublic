...
def diis_engine_helper(self):
    ...
    if core.get_option('SCF', 'DIIS'):
      engines.add('diis')
    if core.get_option('SCF', 'LIST') != "NONE":
        engines.add('list')
...
def _RHF_orbital_gradient(self, save_fock: bool, max_diis_vectors: int, f_ins: list, d_ins: list, v_ins: list) -> float:
    gradient = self.form_FDSmSDF(self.Fa(), self.Da())
    entry = {"target": [self.Fa()]}
    is_mesa = core.get_option('SCF', 'MESA')

    if save_fock or is_mesa:
        if not self.initialized_diis_manager_:
            ...
            S_mat = self.S().clone()     # deep-copy read-only thereafter
            X_mat = S_mat.clone()
            X_mat.power(-0.5, 1.0e-12)    # remove this later for tranform (LIST)

            self.diis_manager_ = DIIS(max_diis_vectors, "HF DIIS vector", RemovalPolicy.LargestError, storage_policy, 
                                      engines=diis_engine_helper(self), X=X_mat, S=S_mat)


            self.initialized_diis_manager_ = True

        if core.get_option('SCF', 'DIIS') or is_mesa:
            entry["error"] = [gradient]
            if len(f_ins) != 0:
                entry["f diff"] = [self.form_FoutmFin(self.Fa().clone(), f_ins[-1].clone())]
            else:
                entry["f diff"] = [self.Fa().clone()]

        aediis = core.get_option('SCF', 'SCF_INITIAL_ACCELERATOR')

        if aediis != "NONE" or is_mesa:
            entry["densities"] = [self.Da()]
            if aediis == "EDIIS" or is_mesa:
                entry["energy"] = [self.compute_E()]

        if core.get_option('SCF', 'LIST') != "NONE" or is_mesa:
            entry["densities"] = [self.Da()]
            entry["potential"] = [self.Ja()]
            if len(f_ins) != 0:
                entry["f diff"] = [self.form_FoutmFin(self.Fa().clone(), f_ins[-1].clone())]
                entry["v diff"] = [self.form_FoutmFin(self.Ja().clone(), v_ins[-1].clone())]
                entry["d diff"] = [self.form_FoutmFin(self.Da().clone(), d_ins[-1].clone())]
                entry["f in"] = [f_ins[-1].clone()]
            else:
                entry["f diff"] = [self.Fa().clone()]
                entry["v diff"] = [self.Ja().clone()]
                entry["d diff"] = [self.Da().clone()]
                # We are getting a matrix with 0s
                entry["f in"] = [self.form_FoutmFin(self.Fa().clone(),
                                                    self.Fa().clone())]
...
def _UHF_orbital_gradient(self, save_fock: bool,
    ...
    if save_fock or is_mesa:
        ...
        entry = {
            "target": [self.Fa(), self.Fb()],
            "error": [gradient_a, gradient_b],
            "densities": [self.Da(), self.Db()],
            "potential": [self.Ja(), self.Jb()]
        }
        ...
        if len(f_in) != 0:
            ...
            last_f = f_in[-1]
            last_d = d_in[-1]
            last_v = v_in[-1] if len(v_in) != 0 else None

            if isinstance(last_f, tuple) and len(last_f) == 2:
                Fa_prev, Fb_prev = last_f

            else:
                #fallback to alpha only
                Fa_prev = last_f
                Fb_prev = self.Fb().clone()  # or zero

            if isinstance(last_d, tuple) and len(last_d) == 2:
                Da_prev, Db_prev = last_d

            else:
                Da_prev = last_d
                Db_prev = self.Db().clone()  # or zero

            if last_v and isinstance(last_v, tuple) and len(last_v) == 2:
                Ja_prev, Jb_prev = last_v
            else:
                Ja_prev = last_v if last_v else self.Ja().clone()
                Jb_prev = self.Jb().clone()

            entry["f diff"] = [self.form_FoutmFin(self.Fa().clone(), Fa_prev.clone()),
                               self.form_FoutmFin(self.Fb().clone(), Fb_prev.clone())]
            entry["d diff"] = [self.form_FoutmFin(self.Da().clone(), Da_prev.clone()),
                               self.form_FoutmFin(self.Db().clone(), Db_prev.clone())]
            entry["v diff"] = [self.form_FoutmFin(self.Ja().clone(), Ja_prev.clone()),
                               self.form_FoutmFin(self.Jb().clone(), Jb_prev.clone())]
            entry["f in"] = [Fa_prev.clone(), Fb_prev.clone()]

        else:
            entry["f diff"] = [self.Fa().clone(), self.Fb().clone()]
            entry["v diff"] = [self.Ja().clone(), self.Jb().clone()]
            entry["d diff"] = [self.Da().clone(), self.Db().clone()]
            entry["f in"] = [self.form_FoutmFin(self.Fa().clone(), self.Fa().clone()),
                             self.form_FoutmFin(self.Fb().clone(), self.Fb().clone())]
        
        # EDIIS needs energy
        if core.get_option('SCF', 'SCF_INITIAL_ACCELERATOR') == "EDIIS" or is_mesa:
            entry["energy"] = [self.compute_E()]
...
def _ROHF_orbital_gradient(self, save_fock: bool, max_diis_vectors: int,
                           f_in:list, d_in:list, v_in:list) -> float:
...
def _RHF_diis(self, Dnorm, *args):
    if args:
        if args[0] == "mesa":
            mesa_method = args[1]
            return self.diis_manager_.extrapolate("mesa", mesa_method, self.Fa(), self.Da(), self.Ja(), Dnorm=Dnorm)
    return self.diis_manager_.extrapolate(self.Fa(), self.Da(), self.Ja(), Dnorm=Dnorm)

def _UHF_diis(self, Dnorm, *args):
    if args and args[0] == "mesa":
        mesa_method = args[1]
        
        alpha_engines = self.diis_manager_.extrapolate("mesa", mesa_method, self.Fa(), self.Da(), self.Ja(), 
                                                     Dnorm=Dnorm, component=0)
         
        beta_engines = self.diis_manager_.extrapolate("mesa", mesa_method, self.Fb(), self.Db(), self.Jb(), 
                                                    Dnorm=Dnorm, component=1)
        
        return alpha_engines.union(beta_engines)
    
    alpha_engines = self.diis_manager_.extrapolate(self.Fa(), self.Da(), self.Ja(), 
                                                 Dnorm=Dnorm, component=0)
    beta_engines = self.diis_manager_.extrapolate(self.Fb(), self.Db(), self.Jb(), 
                                                Dnorm=Dnorm, component=1)
    
    return alpha_engines.union(beta_engines)

def _ROHF_diis(self, Dnorm, *args):
